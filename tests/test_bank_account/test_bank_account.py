...
def transfer_should_raise_error_when_transfering_more_money_than_current_balance(bank_account: BankAccount, bank_account2: BankAccount):
    with pytest.raises(InsufficientFundsError):
        bank_account.transfer(bank_account2, 600)
        
def transfer_should_raise_error_when_transfering_amount_of_zero(bank_account: BankAccount, bank_account2: BankAccount):
    with pytest.raises(ValueError):
        bank_account.transfer(bank_account2, 0)
        
def transfer_should_raise_error_when_transfering_amount_is_negative(bank_account: BankAccount, bank_account2: BankAccount):
    with pytest.raises(ValueError):
        bank_account.transfer(bank_account2, -600)
